  ...
  if ($payee->num_rows > 0) {
      ...
      while ($row = $payee->fetch_assoc()) {
          $link = "?selected_payee={$row['payee']}&office={$row['office']}&address={$row['address']}";
          $payeeData[] = "<tr>
                            <td><a href='{$link}'>{$row['payee']}</a></td>
                            <td><a href='{$link}'>{$row['office']}</a></td>
                            <td><a href='{$link}'>{$row['address']}</a></td>
                            <td>{$row['tin']}</td>
                            <td>{$row['taxable']}</td>
                          </tr>";
      }
  ...
  if($cluster->num_rows > 0){
    ...
    while($row = $cluster->fetch_assoc()){
      $link = "?selected_cluster={$row['cluster']}";
      $clusterData[] = "<tr><td><a href='{$link}'>{$row['cluster']}</a></td></tr>";
    }
    $clusterData[] = "</table>";
  }
  ...
  if($transaction->num_rows > 0){
    ...
    while($row = $transaction->fetch_assoc()){
      $transactionData[] = "<tr><td><a href='?selected_transaction={$row['transaction']}'>{$row['transaction']}</a></td></tr>";
    }
  ...
  $_SESSION['selected_transaction'] = $_GET['selected_transaction'] ?? $_SESSION['selected_transaction'] ?? '';
  $_SESSION['selected_payee'] = $_GET['selected_payee'] ?? $_SESSION['selected_payee'] ?? '';
  $_SESSION['selected_office'] = $_GET['office'] ?? $_SESSION['selected_office'] ?? '';
  $_SESSION['selected_address'] = $_GET['address'] ?? $_SESSION['selected_address'] ?? '';
  $_SESSION['selected_cluster'] = $_GET['selected_cluster'] ?? $_SESSION['selected_cluster'] ?? '';
  ...
  <div class="ms-auto">
    <div class="d-flex flex-row" >
      ...
      <p>Date:</p> <strong class="ms-2"><?php echo $currentDate ?></strong>
      </div>
    <div class="px-5">
    <p>No: </p>
    </div>
    <div class="px-5 d-flex">
    <p >Fund: </p> <strong class="ms-2"><?php echo $_SESSION['selected_cluster'] ?? ''; ?></strong>
    </div>
    </div>

    <div class="d-flex flex-column">
      <p>Transaction: <?php echo $_SESSION['selected_transaction']; ?></p>
      <p>Payee: <?php echo $_SESSION['selected_payee']; ?></p>
      <p>Office: <?php echo $_SESSION['selected_office']; ?></p>
      <p>Address: <?php echo $_SESSION['selected_address']; ?></p>
      <p>Cluster: <?php echo $_SESSION['selected_cluster']; ?></p>
      <p>MFO: </p>
      <p>Account: </p>
      <p>Resp. Center: </p>
      <p>Certified: </p>
